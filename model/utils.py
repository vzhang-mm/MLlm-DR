        ...
        **kwargs,
        ...
        if (
            ...
            if attention_mask.dim() == 2:
                # 如果 attention_mask 是 2D
                position_ids = attention_mask.long().cumsum(-1) - 1
                position_ids.masked_fill_(attention_mask == 0, 1)
                kwargs["position_ids"] = position_ids  # 将 position_ids 放入 kwargs 供后续使用
            elif attention_mask.dim() == 3:######################################################################################################################zw 12.11
                # 1. 提取每行有效性的布尔掩码
                row_validity = attention_mask.sum(dim=-1) > 0
            
                # 2. 初始化 position_ids，直接用 1 填充无效行，避免误判
                position_ids = torch.full_like(row_validity, fill_value=1, dtype=torch.long)
            
                # 3. 生成批次偏移量，每个 batch 对应的行索引范围
                batch_offsets = torch.arange(row_validity.size(0), device=row_validity.device).view(-1, 1) * row_validity.size(1)
            
                # 4. 将二维 row_validity 展平为一维张量，形状为 (batch_size * seq_length,)
                valid_rows_flat = row_validity.flatten()
            
                # 5. 为有效位置生成递增索引
                arange_indices = torch.arange(valid_rows_flat.sum(), device=row_validity.device)
            
                # 6. 初始化展平后的 position_ids
                position_ids_flat = torch.full(valid_rows_flat.size(), fill_value=1, dtype=torch.long, device=row_validity.device)
            
                # 7. 计算有效位置的递增索引并填充
                position_ids_flat[valid_rows_flat] = arange_indices - torch.cumsum(~valid_rows_flat, dim=0)[valid_rows_flat]
            
                # 8. 将展平的 position_ids 恢复到原始形状
                position_ids = position_ids_flat.view(*row_validity.shape)
                # print("Position IDs:", position_ids)
            
                # 9. 无需额外填充，初始值已为 1
            
                # 10. 存入 kwargs，供后续使用
                kwargs["position_ids_3d"] = position_ids
            else:
                raise ValueError(f"Unsupported attention_mask dimension: {attention_mask.dim()}. Expected 2D or 3D.")
            ...
            if "attention_mask" in model_kwargs:
                ...
                if attention_mask.dim() ==2:
                    model_kwargs["attention_mask"] = torch.cat(
                        [attention_mask, attention_mask.new_ones((attention_mask.shape[0], 1))], dim=-1 ########################################新增生成的token mask 值为1
                    )
                elif attention_mask.dim() ==3:
                    # 创建一个新的张量，形状为 (batch_size, sequence_length+1, sequence_length+1)，初始化为0
                    new_attention_mask = torch.ones((attention_mask.shape[0], attention_mask.shape[1] + 1, attention_mask.shape[2] + 1), 
                                                    device=attention_mask.device, dtype=attention_mask.dtype)
                    # 将原始 attention_mask 的值复制到新张量的对应位置上
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            #     if hasattr(v, 'shape'):  # 判断是否有 shape 属性
